...
const BlogItem = async ({ blogItem, className }) => {
    ...
    return (
        <a href={`/blog/${blogItem.id}`} className={`relative flex flex-col items-start ${className} p-3 hover:shadow-md transition-shadow duration-300 ease-in-out`}>
        {
        ...
        <div className="text-sm text-black" dangerouslySetInnerHTML={{ __html: blogItem.introText }}></div>
