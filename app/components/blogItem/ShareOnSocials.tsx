...
const ShareOnSocials: React.FC<ShareOnSocialsProps> = ({ title }) => {
  ...
  const facebookShareLink = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIString(
    fullUrl
  )}`;
  const xShareLink = `https://twitter.com/intent/tweet?text=${encodeURIString(
    title
  )}&url=${encodeURIString(fullUrl)}`;
  const whatsappShareLink = `https://wa.me/?text=${encodeURIString(
    title
