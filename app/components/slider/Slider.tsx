...
const Slider = ({ imagePaths }) => {
    ...
    return (
        <div id="default-carousel" className=" mt-4 relative mx-auto max-w-screen-xl w-full sm:w-[600px] md:w-full" data-carousel="slide" style={{maxWidth: '1000px'}}>
        ...
        <div className="absolute z-30 flex space-x-3 -translate-x-1/2 bottom-5 left-1/2">
            {imagePaths.map((_, index) => (
                <button
                    key={index}
                    type="button"
                    className={`w-3 h-3 rounded-full border border-white bg-white ${currentIndex === index ? 'bg-opacity-100' : 'bg-opacity-50'}`}
